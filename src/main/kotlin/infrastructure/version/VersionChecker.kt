...
class GitHubVersionChecker(
    ...
    private val githubRepository: String = "Xenexes/BragLogBuddy",
) : VersionChecker {
    ...
    override fun checkForUpdates() {
        try {
            ...
            println("BragLogBuddy version: $currentVersion")
            println()
    ...
    private fun printUpdateNotification(
        ...
        println("║${centerText("A new version of BragLogBuddy is available!", boxWidth)}║")
        println("║${" ".repeat(boxWidth)}║")
        println("║  Current version: $currentVersion${" ".repeat(boxWidth - 20 - currentVersion.length)}║")
        println("║  Latest version:  $latestVersion${" ".repeat(boxWidth - 20 - latestVersion.length)}║")
        println("║${" ".repeat(boxWidth)}║")
        println("║  Download: $downloadUrl${" ".repeat(boxWidth - 13 - downloadUrl.length)}║")
        println("╚${"═".repeat(boxWidth)}╝")
        println()
    }
